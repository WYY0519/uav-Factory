          ...
          >
            <div class="routeOperation-box">
              ...
              <div class="zone-info">
                ...
                <div class="info-row info-row-single">
                  <div class="info-item company-item">
                    <!-- 新增company-item类 -->
                    <span class="info-label">公司：</span>
                    <span class="info-value company-value">
                      <!-- 新增company-value类 -->
...
const routeList = async () => {
  try {
    ...
    const searchParams = {
      ...
      currentPage: currentPage.value, // 注意：接口参数为pageNum，原代码currentPage需对应
    ...
    if (res.code === 200) {
      ...
      console.log("更新后的 routeInfo：", routeInfo.value);
      console.log("更新后的 displayedRoutes：", displayedRoutes.value);
...
const handleSizeChange = (val) => {
  ...
  pageSize.value = val;
  currentPage.value = 1; // 修改页大小时重置为第一页
  routeList();
};
...
/* 搜索区域布局：横向排列，间距10px，自适应折行，下拉框与输入框等高 */
.route-search {
  margin-bottom: 12px;
  display: flex;
  gap: 0px;
...
/* 统一搜索框/下拉框样式，与原有深色主题匹配 */
.route-search :deep(.el-input__wrapper),
.route-search :deep(.el-select__wrapper) {
  background-color: #2e3649db;
  border: none;
}
.route-search :deep(.el-input__inner),
.route-search :deep(.el-select__placeholder) {
  color: #fff;
}
.route-search :deep(.el-select__icon) {
...
.info-row {
  display: flex;
  align-items: flex-start; /* 修改为flex-start，让标签和内容顶部对齐 */
  gap: 0;
}
.info-row-between {
  justify-content: space-between;
  align-items: center; /* 总距离行保持居中 */
}
...
.info-item {
  display: flex;
  align-items: flex-start; /* 修改为flex-start，支持换行 */
  gap: 6px;
  flex: 1;
  min-width: 0;
}
...
.company-value {
  ...
  max-width: calc(100% - 60px); /* 限制最大宽度，避免溢出 */
