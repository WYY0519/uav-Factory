<template>
  <div class="company-management">
    ...
    <CommonTable title="公司列表" ref="companyTableRef" :table-data="companyList" :columns="tableColumns" :total="total"
      ...
      <!-- 合作状态列自定义渲染 -->
      <template #col-partnerTime="{ row }">
        <el-tag :type="row.partnerTime ? 'success' : 'info'">
          {{ row.partnerTime ? '合作中' : '未合作' }}
        </el-tag>
      </template>

      <!-- 状态列自定义渲染 -->
      <template #col-status="{ row }">
        <el-tooltip :content="row.status === true ? '禁用公司' : '启用公司'" placement="top">
          <el-switch :model-value="row.status" :active-value="true" :inactive-value="false"
...
const initialSearchData = { companyName: "", companyTaxNumber: "", isPartner: null };
...
const tableColumns = [
  ...
  { prop: "partnerTime", label: "合作状态", width: 100 },
...
const getCompanyList = async () => {
  loading.value = true;
  try {
    const res = await opcompanyListAll({
      pageNum: currentPage.value,
      pageSize: pageSize.value,
      isPartner: formInline.isPartner,
...
// before-change钩子：仅在用户手动点击switch时触发，不会在初始渲染时调用
const handleBeforeStatusChange = async (row) => {
  // 防止id不存在时调用接口
  if (!row.id) {
    console.warn('公司ID不存在，无法操作');
    return false;
  }
  const newVal = !row.status; // true→false(禁用), false→true(启用)
  try {
    let res;
    if (newVal === true) {
      // 启用公司
      res = await companyEnable(row.id);
    } else {
      // 禁用公司
      res = await companyDisable(row.id);
    }
    if (res.code === 200) {
      row.status = newVal;
      ElMessage.success(res.message || (newVal === true ? '公司已启用' : '公司已禁用'));
      return true; // 返回true让el-switch同步切换状态
    } else {
      ElMessage.error(res.message || '操作失败');
      return false; // 返回false阻止切换
    }
  } catch (error) {
